...
import { formatExperienceSpan, formatPeriod, loadResume } from "#/lib/resume/load";
...
function ResumePage() {
  ...
  return (
    <article className="mx-auto min-h-0 w-full max-w-4xl grow overflow-y-auto px-6 py-10 sm:px-10 sm:py-16">
      <header className="flex flex-col gap-6 border-b border-neutral-950/10 pb-8 sm:flex-row sm:items-end sm:justify-between">
        ...
        <a
          className={cn(buttonVariants({ size: "default", variant: "default" }), "shrink-0 rounded-full print:hidden")}
        ...
        <section>
          ...
          <ol className="relative">
            {experiences.map((experience, index) => {
              ...
              return (
                <li key={`${experience.company}-${index}`} className={cn("relative pl-14", isLast ? "pb-0" : "pb-10")}>
                  {!isLast ? (
                    <span aria-hidden="true" className="absolute top-10 bottom-0 left-[19px] w-px bg-neutral-950/10" />
                  ) : null}
                  <CompanyLogo
                  ...
                  <div className="flex flex-wrap items-baseline justify-between gap-x-3">
                    ...
                    <p className="text-xs text-neutral-500">{formatExperienceSpan(experience)}</p>
                  </div>

                  {firstRole ? <RoleBlock hidePeriod={experience.roles.length === 1} role={firstRole} /> : null}

                  {otherRoles.length > 0 ? (
                    <ul className="mt-4 space-y-4">
                      {otherRoles.map((role, roleIndex) => (
                        <li key={roleIndex} className="relative pl-5">
                          <span
                            aria-hidden="true"
                            className="absolute top-[7px] left-0 size-2 rounded-full bg-neutral-300"
                          />
                          <RoleBlock role={role} />
                        </li>
                      ))}
...
function RoleBlock({
  ...
  hidePeriod?: boolean;
  role: ReturnType<typeof loadResume>["experiences"][number]["roles"][number];
}) {
  return (
    <div className="mt-1">
      <div className="flex flex-wrap items-baseline justify-between gap-x-3">
        ...
        {hidePeriod ? null : <p className="text-xs text-neutral-500">{formatPeriod(role.start, role.end)}</p>}
      </div>
