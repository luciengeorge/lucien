...
describe("GET /contact.md", () => {
  it("returns 200 with text/markdown content-type", async () => {
    const res = await getHandler()({ request: new Request("http://localhost/contact.md") });
  ...
  it("carries the canonical url and title in frontmatter, matching the HTML page's SEO metadata", async () => {
    ...
    expect(body).toContain(`url: ${SITE_URL}/contact`);
    expect(body).toContain(`title: ${CONTACT_META.title}`);
    expect(body).toContain(`# ${CONTACT_META.title}`);
  });
